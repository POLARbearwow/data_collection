...
fig.suptitle(f'Trajectory Plot (RPM: {target_rpm})', fontsize=16)
# 为不同文件创建不同的颜色
...
for file_idx, file in enumerate(rpm_files):
    ...
    # 上面的子图绘制轨迹
    axes[file_idx].scatter(-df['new_y'], df['new_x'],
                             color=colors[file_idx],
                             s=20,
                             alpha=0.7)
    
    # 设置轨迹图的标题和标签
    axes[file_idx].set_title(f'File {file_idx+1}')
    axes[file_idx].set_xlabel('New Y Position (Negated)')
    axes[file_idx].set_ylabel('New X Position')
    axes[file_idx].grid(True, linestyle='--', alpha=0.7)
    
    # # 下面的子图绘制时间间隔 - 将 Pandas Series 转换为 NumPy 数组
    # time_diff_array = df['time_diff'].to_numpy()
    # sample_indices = np.arange(len(time_diff_array))
    # axes[1, file_idx].plot(sample_indices, time_diff_array, 'b.')
